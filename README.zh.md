...
# 填写：JWT_SECRET, CLAWPLAY_SECRET_KEY, UPSTASH_REDIS_REST_URL/TOKEN, DATABASE_URL, BASE_URL, SMTP_*（审核邮件）
pnpm install
pnpm dev
```
...
| `TEAM_REVIEW_EMAIL` | 团队审核收件箱（默认：`clawplay-team@googlegroups.com`） |
| `SMTP_HOST` | 审核提醒邮件的 SMTP 服务器地址 |
| `SMTP_PORT` | SMTP 端口（Gmail 常用 `587`） |
| `SMTP_USER` | SMTP 登录账号，通常是你的个人 Gmail/Workspace 邮箱 |
| `SMTP_PASS` | SMTP 应用专用密码 |
| `SMTP_FROM` | 外发邮件的发件人地址 |
| `SMTP_SECURE` | 需要隐式 TLS 时设为 `true`（可选） |
